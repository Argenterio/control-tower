...
db.exec(`
  CREATE TABLE IF NOT EXISTS trip_evidence (
    ...
    description TEXT,
    mediaUrl TEXT,
    mimeType TEXT,                                   -- image/jpeg | audio/ogg | application/pdf | video/mp4
    fileName TEXT,                                   -- nombre en disco (ej: 6b2f...jpg)
...
// Migración: registrar tipo MIME, clase semántica y nombre de archivo del adjunto.
// Esto permite que el frontend distinga JPG vs OGG vs PDF y los reproduzca correctamente.
safeAddColumn("whatsapp_messages", "mimeType", "TEXT");
safeAddColumn("whatsapp_messages", "mediaKind", "TEXT");
safeAddColumn("whatsapp_messages", "fileName", "TEXT");
safeAddColumn("trip_evidence", "mimeType", "TEXT");
safeAddColumn("trip_evidence", "fileName", "TEXT");
...
export type TripEvidence = {
  ...
  description?: string;
  mediaUrl?: string;
  mimeType?: string;       // MIME del adjunto (image/jpeg, audio/ogg, application/pdf, video/mp4)
  fileName?: string;       // Nombre del archivo guardado (ej: 6b2f...jpg)
  transcript?: string;
  metadata?: string; // JSON
...
export type WhatsappMessageRow = {
  ...
  content?: string;
  mediaUrl?: string;
  mimeType?: string;       // MIME del adjunto
  mediaKind?: string;      // image | audio | video | document | sticker
  fileName?: string;       // Nombre del archivo guardado en /api/media
...
export class DatabaseService {
  ...
  createTrip(trip: Omit<Trip, "id" | "createdAt" | "updatedAt">): Trip {
    ...
    const startTime = trip.startTime || null;
    const kmCompleted = trip.kmCompleted ?? 0;
    const estimatedArrival = trip.estimatedArrival || null;
    const kmTotal = trip.kmTotal ?? null;
    const fare = trip.fare ?? null;
    const fuelCost = trip.fuelCost ?? null;
    const notes = trip.notes || null;

    const stmt = this.db.prepare(
      "INSERT INTO trips (id, companyId, vehicleId, driverId, customerId, origin, destination, status, startTime, kmCompleted, kmTotal, fare, fuelCost, estimatedArrival, notes, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    );
    stmt.run(
      id,
      trip.companyId,
  ...
  createWhatsappMessage(msg: {
    ...
    this.db.prepare(`
      INSERT INTO whatsapp_messages (id, companyId, driverId, phone, direction, messageType, content, mediaUrl, mimeType, mediaKind, fileName, interpretedAction, interpretedConfidence, tripId, messageId, processed, processedAt, rawPayload, responseMessage, createdAt)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id, msg.companyId, msg.driverId || null, msg.phone, msg.direction, msg.messageType,
      msg.content || null, msg.mediaUrl || null,
      msg.mimeType || null, msg.mediaKind || null, msg.fileName || null,
      msg.interpretedAction || null,
  ...
  // Marcar mensaje como procesado + persistir interpretación + responseMessage.
  // Se usa al finalizar el pipeline principal para no perder la respuesta si la IA falla.
  markWhatsappMessageProcessed(
    ...
    const r = this.db.prepare(`
      ...
      SET interpretedAction = ?,
          interpretedConfidence = ?,
          responseMessage = ?,
          processed = 1,
          processedAt = ?
      WHERE id = ?
    `).run(interpretedAction, confidence, responseMessage, now, id);
    return r.changes > 0;
  }
  ...
  createTripEvidence(data: {
    ...
    const now = new Date().toISOString();
    this.db.prepare(`
      INSERT INTO trip_evidence (id, companyId, tripId, driverId, kind, title, description, mediaUrl, mimeType, fileName, transcript, metadata, source, sourceMessageId, capturedAt, createdAt)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id,
      data.companyId,
  ...
  getCompanySettings(companyId: string): (CompanySettings & { companyName?: string; companyEmail?: string; companyPhone?: string; companyAddress?: string; companyCuit?: string }) | null {
    const row = this.db.prepare("SELECT * FROM company_settings WHERE companyId = ?").get(companyId) as any;
    if (!row) return null;
    ...
    return {
      ...
      companyCuit: company?.ruci,
    };
  }

  upsertCompanySettings(companyId: string, data: {
    timezone?: string;
    dateFormat?: string;
    currency?: string;
    whatsappNumber?: string;
    ...
    const existing = this.db.prepare("SELECT id FROM company_settings WHERE companyId = ?").get(companyId) as any;
    if (existing) {
      const fields: string[] = ["updatedAt = ?"];
      const values: any[] = [now];
      if (data.timezone !== undefined) { fields.push("timezone = ?"); values.push(data.timezone); }
      if (data.dateFormat !== undefined) { fields.push("dateFormat = ?"); values.push(data.dateFormat); }
      if (data.currency !== undefined) { fields.push("currency = ?"); values.push(data.currency); }
      if (data.whatsappNumber !== undefined) { fields.push("whatsappNumber = ?"); values.push(data.whatsappNumber); }
      if (data.gpsProvider !== undefined) { fields.push("gpsProvider = ?"); values.push(data.gpsProvider); }
      if (fields.length > 1) {
        values.push(existing.id);
        this.db.prepare(`UPDATE company_settings SET ${fields.join(", ")} WHERE id = ?`).run(...values);
      }
    } else {
      const id = crypto.randomUUID();
      this.db.prepare(
        "INSERT INTO company_settings (id, companyId, timezone, dateFormat, currency, whatsappNumber, gpsProvider, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
      ).run(id, companyId, data.timezone || "America/Argentina/Buenos_Aires", data.dateFormat || "DD/MM/YYYY", data.currency || "ARS", data.whatsappNumber || null, data.gpsProvider || "geotab", now, now);
    }

    if (data.companyName !== undefined || data.companyEmail !== undefined || data.companyPhone !== undefined || data.companyAddress !== undefined || data.companyCuit !== undefined) {
      this.updateCompany(companyId, {
        ...
        ...(data.companyCuit !== undefined ? { ruci: data.companyCuit } : {}),
      } as any);
    }
  }
  ...
  getTripTimeline(companyId: string, tripId: string) {
    ...
    const messages = this.db.prepare(`
      SELECT id, phone, messageType, content, mediaUrl, interpretedAction, createdAt, responseMessage
      FROM whatsapp_messages
      WHERE companyId = ? AND tripId = ?
      ORDER BY createdAt ASC
    `).all(companyId, tripId) as Array<{
      id: string; phone: string; messageType: string; content?: string; mediaUrl?: string; interpretedAction?: string; createdAt: string; responseMessage?: string;
    }>;

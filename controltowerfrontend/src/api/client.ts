...
class ApiClient {
  ...
  async sendWhatsappMessage(phone: string, message: string, driverId?: string, tripId?: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      const msg = err.response?.data?.error || err.response?.data?.message || err.message || 'Error al enviar mensaje';
      throw new Error(msg);
    }
  }
  ...
  async getWhatsappStatus(): Promise<{ botPhone?: string; status?: string }> {
    const res = await this.http.get<ApiResponse<any>>('/api/whatsapp/status');
    if (res.data.success && res.data.data) return res.data.data;
    return {};
  }
  ...
  async getCompanySettings(): Promise<any> {
    const res = await this.http.get<ApiResponse<any>>('/api/company-settings');
    if (res.data.success && res.data.data) return res.data.data;
    return {};
  }

  async updateCompanySettings(data: any): Promise<any> {
    const res = await this.http.put<ApiResponse<any>>('/api/company-settings', data);
    if (res.data.success && res.data.data) return res.data.data;
    throw new Error(res.data.error || 'Error al guardar configuración');
  }

...
export function SettingsPage() {
  ...
  useEffect(() => {
    api.getCompanySettings()
      .then(settings => {
        ...
        if (settings.companyCuit) setCuit(settings.companyCuit);
        if (settings.companyEmail) setEmail(settings.companyEmail);
        if (settings.companyPhone) setPhone(settings.companyPhone);
        if (settings.companyAddress) setAddress(settings.companyAddress);
        if (settings.gpsProvider) setGpsProvider(settings.gpsProvider);
        if (settings.timezone) setTimezone(settings.timezone);
        if (settings.whatsappNumber) {
          const raw = settings.whatsappNumber.replace(/[^0-9]/g, "");
          if (raw.startsWith("549") && raw.length === 13) {
            setWhatsappNumber(`+54 9 ${raw.slice(3, 5)} ${raw.slice(5, 9)}-${raw.slice(9)}`);
          } else {
  ...
  const [simDriverPhone, setSimDriverPhone] = useState('+54 9 11 4455-1122');
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 24, maxWidth: 960 }}>
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div className="chart-card">
            ...
            <div>
              <button type="submit" className="btn btn-primary" style={{ padding: '12px 28px', fontSize: 15, marginTop: 16 }} disabled={saving || loading}>
                {saving ? 'Guardando...' : saved ? <><Check size={18} /> Guardado con Éxito</> : <><Save size={18} /> Guardar Configuración</>}
              </button>
            </div>
          </div>
        </form>

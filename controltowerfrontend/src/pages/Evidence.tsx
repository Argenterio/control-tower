...
import {
  ...
  Volume2, ExternalLink, Video, Download
} from 'lucide-react';
...
const KIND_META: Record<string, { label: string; color: string; icon: any }> = {
  ...
  video:    { label: 'Videos',    color: '#f472b6', icon: Video },
...
// SVG placeholder 1x1 (transparente) para no mostrar el fallback roto
const FALLBACK_IMAGE = 'data:image/svg+xml;base64,';
          ...
          return (
            <div key={ev.id} className="evidence-card" style={{ borderLeftColor: meta.color }}>
              ...
              <div className="evidence-body">
                {/* AUDIO */}
                {(ev.kind === 'audio' || isAudioMime(ev.mimeType)) && resolvedUrl && (
                  <div className="evidence-audio">
                    <audio controls preload="metadata" src={resolvedUrl} style={{ width: '100%', maxWidth: 360, height: 32 }}>
                      <source src={resolvedUrl} type="audio/ogg" />
                ...
                {/* IMAGE */}
                {(ev.kind === 'image' || isImageMime(ev.mimeType)) && resolvedUrl && (
                ...
                {/* VIDEO */}
                {(ev.kind === 'video' || isVideoMime(ev.mimeType)) && resolvedUrl && (
                  <div className="evidence-video">
                    <video controls preload="metadata" className="w-full rounded-xl" style={{ maxHeight: 320 }}>
                      <source src={resolvedUrl} type={ev.mimeType || 'video/mp4'} />
                      Tu navegador no soporta el elemento de video.
                    </video>
                  </div>
                )}

                {ev.description && ev.kind !== 'audio' && ev.kind !== 'video' && !isAudioMime(ev.mimeType) && !isVideoMime(ev.mimeType) && (
                  <p className="evidence-desc">{ev.description}</p>
                )}

                {/* DOCUMENT (PDF / ZIP / otros) */}
                {ev.kind === 'document' && resolvedUrl && !isImageMime(ev.mimeType) && !isAudioMime(ev.mimeType) && !isVideoMime(ev.mimeType) && (
                  <div className="evidence-doc-card" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', border: '1px solid #334155', borderRadius: 10, background: '#0f172a' }}>
                    <FileText size={18} color="#fbbf24" />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: '#e2e8f0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {ev.fileName || ev.description || 'Documento adjunto'}
                      </div>
                      <div style={{ fontSize: 11, color: '#94a3b8' }}>{ev.mimeType || 'archivo'}</div>
                    </div>
                    <a className="btn btn-secondary" href={resolvedUrl} target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', fontSize: 12 }}>
